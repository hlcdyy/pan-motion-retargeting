...
def rotate_at_each_frame(X, Q, parents):
    """
    ...
    key_glob_Q = global_q[..., 0:1, :]  # (B, T, 1, 4)

    forward = np.array([1, 0, 1])[np.newaxis, np.newaxis, np.newaxis, :] \
                 * quat_mul_vec(key_glob_Q, np.array([0, 1, 0])[np.newaxis, np.newaxis, np.newaxis, :])  # (B, T, 1, 3)
    forward = normalize(forward)

    # yrot = quat_normalize(quat_between(np.array([1, 0, 0]), forward))
    yrot = quat_normalize(quat_between(np.array([0, 0, 1]), forward))  # (B, T, 1, 4)
    new_glob_Q = quat_mul(quat_inv(yrot), global_q)
    new_glob_X = quat_mul_vec(quat_inv(yrot), global_x - global_x[..., 0:1, :]) + global_x[..., 0:1, :]
    # new_glob_X = quat_mul_vec(quat_inv(yrot), global_x)

    # back to local quat-pos
    Q, X = quat_ik(new_glob_Q, new_glob_X, parents)

    return X, Q, yrot, forward


def rotate_at_each_dog_frame(X, Q, parents):
    """
    Re-orients the animation data according to the last frame of past context.
    ...
    """
    # Get global quats and global poses (FK)
    global_q, global_x = quat_fk(Q, X, parents)  # (B, T, J, 4) (B, T, J, 3)

    # key_glob_Q = global_q[..., 0:1, :]  # (B, T, 1, 4)

    """ Extract Forward Direction """
    forward = (
            (global_x[..., 2, :] - global_x[..., 0, :]))
    forward[..., 1] = 0
    forward = forward / np.sqrt((forward ** 2).sum(axis=-1))[..., np.newaxis]
    """ Smooth Forward Direction """
    direction_filterwidth = 8
    forward = filters.gaussian_filter1d(
        forward, direction_filterwidth, axis=1, mode='nearest')
    forward = forward / np.sqrt((forward**2).sum(axis=-1))[...,np.newaxis] # B T 3
    forward = forward[...,np.newaxis, :] # B T 1 3
...
def extract_local_velocities(vel, yrot):
    """
    calculate velocities of whole joints after facing on (0, 0, 1)
    :param vel: velocities of joints B * T * J * 3
    :param yrot:  return by function rotate_at_frame
    :return:
    """
    local_velocities = quat_mul_vec(quat_inv(yrot), vel)
