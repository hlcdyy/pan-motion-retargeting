...
def batch_mat_copy(source_path, suffix, dest_path, dest_suffix=None):
    try_mkdir(dest_path)
    files = [f for f in os.listdir(source_path) if f.endswith('_{}_gt.mat'.format(suffix))]

    length = len('_{}_gt.mat'.format(suffix))
    for f in files:
        if dest_suffix is not None:
            cmd = 'cp \"{}\" \"{}\"'.format(os.path.join(source_path, f), os.path.join(dest_path, f[:-length] + '_{}.mat'.format(dest_suffix)))
        else:
            cmd = 'cp \"{}\" \"{}\"'.format(os.path.join(source_path, f), os.path.join(dest_path, f[:-length] + '.mat'))
        os.system(cmd)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_dir', type=str, default='./pretrained_mixamo/')
    ...
    for i in range(4):
        ...
        if args.device == 'cpu':
            eval(i, prefix, "cpu", epoch=args.epoch)
        else:
            eval(i, prefix, "cuda:0", epoch=args.epoch)
        ...
        if i == 0:
            ...
            for char in test_characters:
                ...
                batch_mat_copy(os.path.join(source_path, char), 0, os.path.join(cross_dest_path, char), 'gt')
        ...
        for char in test_characters:
            for char in test_characters:
                ...
                batch_mat_copy(os.path.join(source_path, char), 1, os.path.join(intra_dest, char), 'gt')
