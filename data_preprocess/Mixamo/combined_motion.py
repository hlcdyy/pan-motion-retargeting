...
import copy
from data_preprocess.Mixamo.motion_dataset import MotionData
import os
import numpy as np
import torch
from data_preprocess.Mixamo.bvh_parser import BVH_file
from parser.parser_mixamo import get_std_bvh
from data_preprocess.Mixamo import get_test_set
...
class MixedData(Dataset):
    ...
    def __init__(self, args, datasets_groups):
        ...
        for datasets in datasets_groups:
            ...
            for i, dataset in enumerate(datasets):
                ...
                tmp.append(MotionData(new_args))      # append one character motion

                mean = np.load('./data_preprocess/Mixamo/Mixamo/mean_var/{}_mean.npy'.format(dataset))
                var = np.load('./data_preprocess/Mixamo/Mixamo/mean_var/{}_var.npy'.format(dataset))
            ...
            all_datas.append(tmp)   # list contains groups of skeleton motions
    ...
    def __getitem__(self, item):
        ...
        for data in self.final_data:
            res.append(data[item])      #The biger datasets_group cant get whole items
        return res
...
class TestData(Dataset):
    def __init__(self, args, characters):
        ...
        for i, character_group in enumerate(characters):
            ...
            for j, character in enumerate(character_group):
                ...
                # print(file.skeleton_type, character)
                if j == 0:
                    self.joint_topologies.append(file.topology)
                    self.ee_ids.append(file.get_ee_id())
                    self.inverse_simplify_maps.append(file.inverse_simplify_map)
                    self.simplified_names.append(file.simplified_name)
                new_offset = file.offset
                new_offset = torch.tensor(new_offset, dtype=torch.float)
                new_offset = new_offset.reshape((1,) + new_offset.shape)

                offsets_group.append(new_offset)
                mean = np.load('./data_preprocess/Mixamo/Mixamo/mean_var/{}_mean.npy'.format(character))
                var = np.load('./data_preprocess/Mixamo/Mixamo/mean_var/{}_var.npy'.format(character))
                mean = torch.tensor(mean)
                mean = mean.reshape((1, ) + mean.shape)
                var = torch.tensor(var)
                var = var.reshape((1, ) + var.shape)
    ...
    def get_item(self, gid, pid, id):
        ...
        path = './data_preprocess/Mixamo/Mixamo/{}/'.format(character)
        ...
        file = BVH_file(file, args=self.args)
