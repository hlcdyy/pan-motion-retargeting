...
from parser.parser_mixamo import get_args, try_mkdir
...
def collect_bvh(data_path, character, files):
    ...
    for i, motion in enumerate(files):
        if not os.path.exists(data_path + character + '/' + motion):
            continue
        file = BVH_file(data_path + character + '/' + motion, args=args)
...
def copy_std_bvh(data_path, character, files):
    ...
    """
    cmd = 'cp \"{}\" ./data_preprocess/Mixamo/Mixamo/std_bvhs/{}.bvh'.format(data_path + character + '/' + files[0], character)
    os.system(cmd)


if __name__ == '__main__':
    prefix = './data_preprocess/Mixamo/Mixamo/'
    ...
    for character in characters:
        ...
        write_statistics(character, './data_preprocess/Mixamo/Mixamo/mean_var/')
