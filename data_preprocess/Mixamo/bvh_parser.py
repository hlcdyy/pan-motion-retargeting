import torch
import outer_utils.BVH_mod as BVH
import numpy as np
from outer_utils.Quaternions import Quaternions
from models.Kinematics import ForwardKinematics
from utils.utils import build_edge_topology
from parser.parser_mixamo import get_std_bvh
from data_preprocess.Mixamo.bvh_writer import write_bvh
...
corps_name_h36m = ['Hips', 'LeftUpLeg', 'LeftLowLeg', 'LeftFoot', 'LeftToeBase', 'RightUpLeg', 'RightLowLeg', 'RightFoot', 'RightToeBase', 'Spine', 'Spine1', 'Neck', 'Head', 'LeftShoulder', 'LeftUpArm', 'LeftForeArm', 'LeftHand', 'RightShoulder', 'RightUpArm', 'RightForeArm', 'RightHand']
"""
2.
...
corps_names.append(corps_name_h36m)
...
class BVH_file:
    def __init__(self, file_path=None, args=None, dataset=None, new_root=None):
        ...
        if self.skeleton_type == 0 and len(self._names) > 30:
            self.set_new_root(1)
        else:
            self.anim.parents[0] = 0
        ...
        self.virtual_joints = [(i, self.simplify_map[i+1]) for i, name in enumerate(self._names) if name not in
                               corps_names[self.skeleton_type] and "virtual" in name]

        self.edges = build_edge_topology(self.topology, self.offset)  # len(self.topology) = joint_num, self.edges = joint_num -1
    ...
    def topology(self):
        if self._topology is None:
            ...
            for i in range(self._topology.shape[0]):
                if i >= 1 and self.simplify_map[self._topology[i]] != -1:
                    self._topology[i] = self.simplify_map[self._topology[i]]
                elif i >= 1 and self.simplify_map[self._topology[i]] == -1:
                    # a = self.simplify_map[self.anim.parents[self._topology[i]]]
                    self._topology[i] = self.simplify_map[self.anim.parents[self._topology[i]]]
    ...
    def to_numpy(self, quater=False, edge=True, order='xyz'):
        rotations = self.anim.rotations[:, self.corps, :]
        if len(self.virtual_joints) != 0:
            virtual_rotations = self.anim.rotations[:, [value[0] for value in self.virtual_joints], :]
        if quater:
            rotations = Quaternions.from_euler(np.radians(rotations), order).qs
            if len(self.virtual_joints) != 0:
                virtual_rotations = Quaternions.from_euler(np.radians(virtual_rotations), order).qs
        ...
        if edge:
            ...
            if len(self.virtual_joints) != 0:
                for i, value in enumerate(self.virtual_joints):
                    if quater:
                        multi = Quaternions(rotations[:, value[1] - 1, :]) * Quaternions(virtual_rotations[:, i, :])
                        rotations[:, value[1] - 1, :] = multi.qs
                    else:
                        rotations = Quaternions.from_euler(np.radians(rotations), order).qs
                        virtual_rotations = Quaternions.from_euler(np.radians(virtual_rotations), order).qs
                        multi = Quaternions(rotations[:, value[1] - 1, :]) * Quaternions(virtual_rotations[:, i, :])
                        rotations[:, value[1] - 1, :] = multi.qs
                        rotations = np.degrees(Quaternions(rotations).euler())

        rotations = rotations.reshape(rotations.shape[0], -1)

        return np.concatenate((rotations, positions), axis=1)

    def to_tensor(self, quater=False, edge=True, order='xyz'):
        res = self.to_numpy(quater, edge, order)
