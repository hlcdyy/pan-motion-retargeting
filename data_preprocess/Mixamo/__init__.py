def get_character_names(args):
    if args.is_train:
        """
        Put the name of subdirectory in ./data_preprocess/Mixamo/Mixamo as [[names of group A], [names of group B]]
        """
...
def create_dataset(args, character_names=None):
    from data_preprocess.Mixamo.combined_motion import TestData, MixedData
...
def get_test_set():
    with open('./data_preprocess/Mixamo/Mixamo/test_list.txt', 'r') as file:
        list = file.readlines()
...
def get_train_list():
    with open('./data_preprocess/Mixamo/Mixamo/train_list.txt', 'r') as file:
        list = file.readlines()
        list = [f[:-1] for f in list]
        return list
